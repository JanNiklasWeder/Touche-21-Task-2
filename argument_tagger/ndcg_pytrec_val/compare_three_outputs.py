...
def main():
    ...
    parser.add_argument('third_approach')
    ...
    assert os.path.exists(args.third_approach)
    ...
    with open(args.third_approach, 'r') as f_run:
        third_run = pytrec_eval.parse_run(f_run)
    ...
    third_results = evaluator.evaluate(third_run)
    ...
    third_nDCGs = []
    third_query_value = {}
    for query_id, query_measures in third_results.items():
        for measure, value in sorted(query_measures.items()):
            third_nDCGs.append(value)
            #print_line(measure, query_id, value)
            third_query_value[query_id] = value
    print(' avg of second aprroach nDCG {:f}'.format(mean(third_nDCGs)))
...
'''
    fig3 = go.Figure(
        data=[
            ...
            go.Scatter(
                name="third approach",
                x=list(third_query_value.keys()),
                y=list(third_query_value.values()),
                mode='lines+markers',
                #offsetgroup=1,
            ),
        ],
        layout=go.Layout(
            title="comparing first, second, third approach",
            yaxis_title="DCG value"
        )
    )
    fig3.show()
    ...
    for key, value in first_query_value.items():
        if third_query_value[key]>value:
            better_queries.append(key)
    print(better_queries)
