...
def get_similar_sensevec(data):
    #data here is a topic 
    #by apply for touche py-file of sense2vec will be used: similar_words_by_spacy.py
    #here load from local file to save time
    def read_text(file_name):
        f = open(file_name+".txt", "r")
        return [e for e in f.read().split("\n\n") if len(e)!=0]
    expanded_topics = read_text("expanded_topics")
    for e in expanded_topics:
        if data in e:
            print(data)
            print(e)
            return e
