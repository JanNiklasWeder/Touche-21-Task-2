...
def pandas_to_qrel(
    data: pandas.DataFrame, save_dir: Path, name: str = "Undefined"
) -> None:
    ...
    for topic_id in topic_ids:
        rank = 1
        data_for_topic = data.loc[data["TopicID"] == topic_id].sort_values(
...
if __name__ == "__main__":
    ...
    topics = Path(
        "/home/jan/Uni/AdvancedInformationRetrival/touche/Bert_Docker/res/topics-task-2.xml"
    )
    chatnoir_res = Path(
        "/home/jan/Uni/AdvancedInformationRetrival/touche/ChatNoir/res/100.csv"
    )
    fulltext = Path(
        "/home/jan/Uni/AdvancedInformationRetrival/touche/Bert_Docker/res/UUID-Text.csv"
    )
    name = "Bert"
    model_dir = Path(
        "/home/jan/Uni/AdvancedInformationRetrival/touche/Bert_Docker/saves/CrossValidation"
    )
    ...
    for i in range(1, 51):
        ...
        model = model = ClassificationModel("bert", path)
    ...
    data = pandas.merge(
        ...
        predictions_df,
        how="inner",
        left_on=["Topic", "FullText"],
        right_on=["text_a", "text_b"],
    )
    print(data)
